  ...
  return (
    <div className="w-full min-h-screen py-3 px-2">
      <div className="w-full max-w-[600px] m-auto flex flex-col gap-4">
        {data?.map((info, index) => {
          const total = () => {
            const descount = info.months == 2 ? 5 : info.months == 4 ? 10 : 0;
            const platforms =
              info.disneyProfiles * (25 - descount) +
              info.hboProfiles * (25 - descount) +
              info.primeProfiles * (25 - descount) +
              info.paramountProfiles * (25 - descount) +
              info.starProfiles * (25 - descount);
            return platforms * info.months;
          };
          ...
          return (
            <div key={index} className="bg-[#F1F1F1] p-2">
              ...
              <div className="flex gap-2">
                ...
                <a className={`font-Arvo text-gray-900 underline`} target="_blank" href={info.imgURL}>
                  enlace/
                </a>
              </div>
            </div>;
